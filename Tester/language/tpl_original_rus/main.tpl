...
<!--
	...
	function TheEnd(){
		if (window.confirm('Вы уверены, что хотите получить оценку?'))
		{
			GetResult();
		}
		return true;
...
function right(e)
{
	...
	else if (navigator.appName == 'Microsoft Internet Explorer' && (event.button == 2 || event.button == 3))
	{
		...
		{
			alert ("Во время сдачи экзамена нажатия правой кнопки мыши запрещено. Если это повторится, для вас тест БУДЕТ ЗАКОНЧЕН. Будет выставленная Ваша текущая оценка.");
		}
		return false;
	}
	return true;
...
Имя студента: <a class=data> {$STNAME} </a><br />
Группа: <a class=data> {$GRNAME} </a><br />
Название теста: <a class=data> {$TESTNAME} </a><br />
{$N_CURRENT} задание с {$N_MAX}<br />
Стоимость задачи {$POINT} бал(ов).
До конца осталось: <input type="text" name="r_t_min" value="{$R_T_MIN}" size="2" readonly> минут 
<input type="text" name="r_t_sec" value="{$R_T_SEC}" size="2" readonly > секунд. <br><br>
  <table width="95%" border="2" bordercolordark=ffffff bordercolor="dedede" cellpadding="2" cellspacing="2" align="center"> 
    <tr align="center">
      <td colspan="2" class=block>Задание</td>
    </tr>
    <tr>
...
{if $Q_TYPE == 1 || $Q_TYPE == 2}
      <td colspan="2" height="30">Варианты ответов</td>
{elseif $Q_TYPE == 3}
      <td colspan="2" height="30">Введите ответ</td>
{/if}
    </tr>
...
<button type="button" onclick="canGetResult=false;return TheEnd();">Закончить тестирование</button>
</td></tr>
</table>
</form>
